...
func (uc *UpstreamController) Start() error {
	...
	uc.podDeleteChan = make(chan model.Message, config.Get().DeletePodBuffer)
	...
	for i := 0; i < config.Get().DeletePodWorkers; i++ {
		go uc.deletePod()
	}
	return nil
}
...
func (uc *UpstreamController) deletePod() {
	for {
		...
		case msg := <-uc.podDeleteChan:
			...
			err = uc.kubeClient.CoreV1().Pods(namespace).Delete(name, &metaV1.DeleteOptions{})
			if errors.IsNotFound(err) {
